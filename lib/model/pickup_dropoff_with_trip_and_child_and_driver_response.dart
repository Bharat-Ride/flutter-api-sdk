...
class PickupDropoffWithTripAndChildAndDriverResponse {
  ...
  static PickupDropoffWithTripAndChildAndDriverResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PickupDropoffWithTripAndChildAndDriverResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PickupDropoffWithTripAndChildAndDriverResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return PickupDropoffWithTripAndChildAndDriverResponse(
  ...
  static List<PickupDropoffWithTripAndChildAndDriverResponse>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PickupDropoffWithTripAndChildAndDriverResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PickupDropoffWithTripAndChildAndDriverResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PickupDropoffWithTripAndChildAndDriverResponse> mapFromJson(dynamic json) {
    final map = <String, PickupDropoffWithTripAndChildAndDriverResponse>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PickupDropoffWithTripAndChildAndDriverResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<PickupDropoffWithTripAndChildAndDriverResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PickupDropoffWithTripAndChildAndDriverResponse>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PickupDropoffWithTripAndChildAndDriverResponse.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
